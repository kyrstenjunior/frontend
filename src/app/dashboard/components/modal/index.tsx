        ...
        <article>
          ...
          {order.map(item => (
            <section className="flex flex-col my-4 text-base" key={item.id}>
              <span>
                Qtd: {item.amount} - <strong>{ item.product.name }</strong> - R$ {parseFloat(item.product.price) * item.amount}
              </span>
              <span className="text-gray-300 mt-1">{ item.product.description }</span>
            </section>
          ))}

          <h3 className="mt-2 mb-4 font-bold text-lg">Valor total: R$ {calculateTotalOrder(order)}</h3>
